...
// HP-balk met kleur per gezondheidsgraad
void print_hp_bar(const char* label, int hp, int max_hp) {
    ...
    const char* color;
    if (ratio > 0.6) color = "\033[1;32m";       // groen
    else if (ratio > 0.3) color = "\033[1;33m";  // geel
    else color = "\033[1;31m";                   // rood
...
void fight(Player* player, Room* room) {
    ...
    while (player->hp > 0 && monster->hp > 0) {
        ...
        for (int i = 4; i >= 0; i--) {
            ...
            // Toon actuele HP's vóór elke actie
